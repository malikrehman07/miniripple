...
const PricingSection = () => {
        ...
        const fetchPlans = async () => {
            try {
                ...
                if (data.plans.length > 0) {
                    setSelectedPlanType(data.plans[0].PlanType.name); // Default selection
                }
            } catch (error) {
    ...
    const uniquePlanTypes = [...new Set(plans.map(plan => plan.PlanType.name))];

    const filteredPlans = plans.filter(plan => plan.PlanType.name === selectedPlanType);

    const getDurationLabel = (days) => {
        if (days === "lifetime") return "lifetime";
        const numDays = Number(days || 0);
        return numDays >= 360 ? "yr" : "mo";
    };

    const handleGetStarted = async (planId) => {
        localStorage.setItem("selectedPlanId", planId);
        navigate(`/terms?planId=${planId}`);
        // try {
        //     if (!user) {
        //         // navigate("/login");
        //         // return;
    ...
    return (
        <section id="pricing" className="relative z-10 w-full pt-2 md:pt-32">
            <div className="flex flex-col items-center md:px-24 pb-8">
                ...
                <div className="flex flex-wrap gap-3 items-center justify-center p-3 mt-12 text-base font-semibold leading-none rounded-2xl bg-blue-700 bg-opacity-20 font-['Jost']">
                    {uniquePlanTypes.map((planTypeName, index) => (
                        <Button
                            key={index}
                            onClick={() => setSelectedPlanType(planTypeName)}
                            variant={selectedPlanType === planTypeName ? "primary" : "secondary"}
                        >
                            {planTypeName}
                        </Button>
                    ))}
