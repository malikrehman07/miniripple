...
function UserListTable({ data, loading }) {
    ...
    return (
        <div className="flex overflow-hidden flex-wrap items-start mt-6 mr-6 ml-6 rounded-md shadow-sm bg-neutral-50">
            <TableColumn 
                title="Name" 
                items={data.map((user) => formatName(user.firstName, user.lastName))} 
                emptyCount={emptyRows} 
            />
            <TableColumn 
                title="Email" 
                items={data.map((user) => user.email)} 
                emptyCount={emptyRows} 
            />
            <TableColumn 
                title="Role" 
                items={data.map((user) => user.role || "N/A")} 
                emptyCount={emptyRows} 
            />
            <TableColumn 
                title="Phone Number" 
                items={data.map((user) => user.phoneNumber || "N/A")} 
                emptyCount={emptyRows} 
            />
            <TableColumn 
                title="Added Date" 
                items={data.map((user) => formatDate(user.createdAt))} 
                width="w-[185px]" 
                emptyCount={emptyRows} 
            />
            <TableColumn
...
function ActionsColumn({ rowCount, emptyCount, onUserClick, users }) {
                ...
                >
                    <div className="flex flex-1 shrink gap-2.5 items-center self-stretch px-6 py-[19px] my-auto w-full rounded-none basis-0 max-md:px-5">
                        <div className="flex items-start self-stretch my-auto w-9">
                            <button 
                                className="flex gap-2.5 justify-center items-center px-1.5 py-1.5 w-9 rounded shadow-sm bg-neutral-50 hover:bg-blue-100 transition-colors cursor-pointer"
                                onClick={() => onUserClick(users[index]._id)}
