...
const BlockedCountryManager = () => {
  ...
  const handleSelect = async (countryCode) => {
    if (selectedCountries.includes(countryCode)) {
      notification.error("This country is already blocked.");
      return;
    }
  ...
  const handleUnblock = async (countryCode) => {
    try {
      ...
      fetchBlockedCountries();
      notification.success("Country unblocked successfully!");
    } catch (error) {
      let msg = "Failed to unblock country.";
      if (error.response && error.response.data) {
        msg = error.response.data.message || error.response.data.error || msg;
      }
      notification.error(msg);
