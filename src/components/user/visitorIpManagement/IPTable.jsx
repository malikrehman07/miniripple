...
function IPTable({ data, onRemoveBlockedCountry, onRemoveBlockedIP }) {
    ...
    return (
        <div className="flex overflow-hidden flex-wrap items-start mt-6 mr-6 ml-6 rounded-md shadow-sm bg-neutral-50 hehehe">
            <TableColumn title="Visitor Id" items={data.map((item) => item.id)} emptyCount={emptyRows} />
            ...
            <TableColumn title="City" items={data.map((item) => item.city)} emptyCount={emptyRows} />
            <TableColumn title="Country" items={data.map((item) => item.country)} emptyCount={emptyRows} />
                    ...
                    <div className="flex gap-2.5 items-center px-6 py-[19px]">
                        <button
                            onClick={() => {
                                ...
                                } else if (onRemoveBlockedCountry && row.countryCode) {
                                    onRemoveBlockedCountry(row.countryCode);
                                }
                            }}
