...
const TrafficTrendsChart = ({
    ...
    useEffect(() => {
        ...
        try {
            ...
            gaData.forEach((row) => {
                if (!row.dateHour) {
                    console.warn("Missing dateHour in row:", row);
                    return;
                }

                let fullDate;
                if (row.dateHour.length === 8) {
                    // Format is yyyyMMdd (no hour)
                    fullDate = parse(row.dateHour, "yyyyMMdd", new Date());
                } else if (row.dateHour.length === 10) {
                    // Format is yyyyMMddHH (with hour)
                    fullDate = parse(row.dateHour, "yyyyMMddHH", new Date());
                } else {
                    console.warn("Unexpected dateHour format:", row.dateHour, "in row:", row);
                    return;
                }

                if (isNaN(fullDate)) {
                    console.warn("Invalid dateHour value:", row.dateHour, "in row:", row);
                    return;
                }
