...
export default function VisualRepresentationMap() {
    ...
    const { countries: gaData } = useVisitorsByCountry();
    ...
    useEffect(() => {
        ...
        if (Array.isArray(gaData)) {
            for (const row of gaData) {
                ...
                if (!visitorMap[countryCode]) visitorMap[countryCode] = 0;
                visitorMap[countryCode] += count;
            }
        }
                ...
                <Geographies geography={geoUrl}>
                    {({ geographies }) => {
                        return geographies.map((geo) => {
                            const countryCode = geo.properties.iso_a2;  // Get the ISO 2-letter country code from geo data
                            const name = geo.properties.name;
                            const userCount = countryVisitorMap[countryCode] || 0;  // Match using country code
