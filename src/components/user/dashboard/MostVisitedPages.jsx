...
const MostVisitedPagesChart = ({ gaData = [] }) => {
  ...
  useEffect(() => {
    ...
    for (const row of gaData) {
      ...
      const pageName = row.pageName;  // Corrected pageName assignment

      if (!path || isNaN(views)) continue;

      // Correctly associate pageName with the path
      pageViewsMap[path] = {
        name: pageName,    // Store pageName
        views: (pageViewsMap[path]?.views || 0) + views,
      };
    }
